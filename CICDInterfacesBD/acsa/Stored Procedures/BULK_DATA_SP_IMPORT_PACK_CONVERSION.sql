...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [acsa].[BULK_DATA_SP_IMPORT_PACK_CONVERSION]
*/
-- =============================================
CREATE PROCEDURE [acsa].[BULK_DATA_SP_IMPORT_PACK_CONVERSION]
AS
BEGIN
  SET NOCOUNT ON;
  --
  MERGE SWIFT_EXPRESS_R.acsa.SONDA_PACK_UNIT AS TRG
  USING (SELECT
      epu.CODE_PACK_UNIT
     ...
     ,epu.LAST_UPDATE_BY
     ,epu.UM_ENTRY
    FROM SWIFT_INTERFACES_ONLINE_R.acsa.ERP_PACK_UNIT epu) AS SRC
  ...
  USING (SELECT
     ...
     ,epc.LAST_UPDATE_BY
     ,epc.[ORDER]
    FROM SWIFT_INTERFACES_ONLINE_R.acsa.ERP_PACK_CONVERSION epc) AS SRC
