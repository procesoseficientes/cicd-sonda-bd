...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [acsa].[BULK_DATA_SP_PROCESS]
				--
				SELECT TOP 25 * FROM acsa.[BULK_DATA_CONFIGURATION_LOG] ORDER BY 1 DESC
*/
-- =============================================
CREATE PROCEDURE [acsa].[BULK_DATA_SP_PROCESS]
AS
BEGIN
	SET NOCOUNT ON;
	--
	...
	SELECT 
		...
		,C.[ORDER]
	INTO #CONFIG
	FROM acsa.[BULK_DATA_CONFIGURATION] C
	WHERE C.ACTIVE = 1
	ORDER BY 
		C.[ORDER]
		,C.CONFIGURATION
	...
	BEGIN
		...
		SET @END_RUN = GETDATE()
		--
		EXEC acsa.[BULK_DATA_SP_INSERT_LOG]
			@SP = @SP
			,@START_RUN = @START_RUN
			,@END_RUN = @END_RUN
			,@MESSAGE = @MESSAGE
