...
-- Ejemplo de Ejecucion:
			EXEC [acsa].[BULK_DATA_SP_GENERATE_PRICE_LIST_FOR_ROUTE]
*/
-- =============================================
CREATE PROCEDURE [acsa].[BULK_DATA_SP_GENERATE_PRICE_LIST_FOR_ROUTE]
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  IF EXISTS (SELECT
        *
      FROM [SWIFT_EXPRESS_R].[acsa].[SWIFT_PARAMETER] [SP]
  ...
  BEGIN
    ...
    -- ------------------------------------------------------------------------------------
    TRUNCATE TABLE [SWIFT_EXPRESS_R].[acsa].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE]
    ...
    INSERT INTO @ROUTES ([CODE_ROUTE])
      SELECT DISTINCT
        [SR].[CODE_ROUTE]
      FROM [SWIFT_EXPRESS_R].[acsa].[SWIFT_ROUTES] [SR]
      INNER JOIN [SWIFT_EXPRESS_R].[acsa].[USERS] [U]
        ON [SR].[CODE_ROUTE] = [U].[SELLER_ROUTE] collate database_default
    ...
    BEGIN
      ...
      EXEC [SWIFT_EXPRESS_R].[acsa].[SWIFT_SP_GENERATE_PRICE_LIST_FOR_ROUTE] @CODE_ROUTE =@CODE_ROUTE
