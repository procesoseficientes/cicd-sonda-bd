...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [acsa].[BULK_DATA_SP_IMPORT_PRICE_LIST]
*/
-- =============================================
CREATE PROCEDURE [acsa].[BULK_DATA_SP_IMPORT_PRICE_LIST]
AS
BEGIN
	SET NOCOUNT ON;
	--
	MERGE  [SWIFT_EXPRESS_R].[acsa].[SWIFT_PRICE_LIST] AS [TGR]
	USING (SELECT * FROM  [SWIFT_INTERFACES_ONLINE_R].[acsa].[ERP_PRICE_LIST]) AS [SRC] 
	ON [TGR].[CODE_PRICE_LIST] =  [SRC].[CODE_PRICE_LIST] COLLATE DATABASE_DEFAULT
	WHEN  MATCHED THEN 
	UPDATE 
		SET
