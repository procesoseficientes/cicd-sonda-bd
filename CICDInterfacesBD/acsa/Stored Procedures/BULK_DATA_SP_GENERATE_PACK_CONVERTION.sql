...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[BULK_DATA_SP_GENERATE_PACK_CONVERTION]
*/
-- =============================================
CREATE PROCEDURE [acsa].[BULK_DATA_SP_GENERATE_PACK_CONVERTION]
AS
BEGIN
	SET NOCOUNT ON;
	...
	-- ------------------------------------------------------------------------------------
	MERGE [SWIFT_EXPRESS_R].[acsa].[SONDA_PACK_UNIT] AS [SPU]
	USING
		(
			SELECT
	...
	-- ------------------------------------------------------------------------------------
	MERGE [SWIFT_EXPRESS_R].[acsa].[SONDA_PACK_CONVERSION] AS [TRG]
	USING
		(
			SELECT
				[epc].[CODE_SKU]
				...
				,1 [ORDER]
			FROM [SWIFT_EXPRESS_R].[acsa].[SWIFT_VIEW_ALL_SKU] [epc]
		) AS [SRC]
