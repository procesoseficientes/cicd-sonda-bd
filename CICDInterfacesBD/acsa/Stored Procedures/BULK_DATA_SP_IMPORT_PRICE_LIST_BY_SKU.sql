...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [acsa].[BULK_DATA_SP_IMPORT_PRICE_LIST_BY_SKU]
				--
				SELECT * FROM [SWIFT_EXPRESS_R].[acsa].[SWIFT_PRICE_LIST_BY_SKU]
*/
-- =============================================
CREATE PROCEDURE [acsa].[BULK_DATA_SP_IMPORT_PRICE_LIST_BY_SKU]
AS
BEGIN
	SET NOCOUNT ON;
	--
	MERGE [SWIFT_EXPRESS_R].[acsa].[SWIFT_PRICE_LIST_BY_SKU]   [TGR] 
	USING (SELECT * FROM  [SWIFT_INTERFACES_ONLINE_R].[acsa].[ERP_PRICE_LIST_BY_SKU] WHERE [CODE_PRICE_LIST] IS NOT NULL) [SRC] 
	ON  
