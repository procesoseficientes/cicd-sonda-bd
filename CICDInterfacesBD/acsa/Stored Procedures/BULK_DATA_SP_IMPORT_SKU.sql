...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [acsa].[BULK_DATA_SP_IMPORT_SKU]
				--
				SELECT * FROM [acsa].[SWIFT_ERP_SKU]
*/
-- =============================================
CREATE PROCEDURE [acsa].[BULK_DATA_SP_IMPORT_SKU]
AS
BEGIN
  SET NOCOUNT ON;
  --
  DECLARE 
	...
	TRUNCATE TABLE [acsa].[SWIFT_ERP_SKU]

	-- ------------------------------------------------------------------------------------
  -- Obtiene los productos
  -- ------------------------------------------------------------------------------------
	MERGE [acsa].[SWIFT_ERP_SKU] [TRG]
	USING
		(SELECT * FROM [SWIFT_INTERFACES_ONLINE_R].[acsa].[ERP_VIEW_SKU]) AS [SRC]
    ...
    -- ------------------------------------------------------------------------------------
    SELECT @CODEE_PRICE_LIST = SWIFT_EXPRESS_R.[acsa].[SWIFT_FN_GET_PARAMETER]('ERP_HARDCODE_VALUES','PRICE_LIST')
    --
    SELECT @NAME_PRICE_LIST = epl.NAME_PRICE_LIST
    FROM SWIFT_INTERFACES_ONLINE_R.[acsa].ERP_PRICE_LIST epl

    -- ------------------------------------------------------------------------------------
    -- Obtiene coloca el nombre de la lista de preicos
    -- ------------------------------------------------------------------------------------
    UPDATE [acsa].SWIFT_ERP_SKU
