...
SELECT
	...
	,[VALUE_TEXT_CLASSIFICATION]  AS [VALUE_TEXT_CLASSIFICATION]
	,[BARCODE_SKU]  AS [BARCODE_SKU]
	,[CODE_PROVIDER]  AS [CODE_PROVIDER]
	,NULL AS 'LIST_PRICE'
	,NULL AS 'COST'
	,[MEASURE]  AS [MEASURE] --CASE S.HANDLE_BATCH WHEN '' THEN '0' ELSE S.HANDLE_BATCH END AS MEASURE,
	,[ERP].[LAST_UPDATE] AS [LAST_UPDATE]
	,[ERP].[LAST_UPDATE_BY]  AS [LAST_UPDATE_BY]
	,[HANDLE_SERIAL_NUMBER] AS [HANDLE_SERIAL_NUMBER]
	,[HANDLE_BATCH]  AS [HANDLE_BATCH]
	,
  --0 AS ON_HAND,
	[FROM_ERP] [FROM_ERP]
	,[CODE_FAMILY_SKU]  AS [CODE_FAMILY_SKU]
	...
	,ERP.VAT_CODE
FROM
	[$(CICDInterfacesBD)].[acsa].[ERP_VIEW_SKU] [ERP]
...
LEFT JOIN [acsa].[SWIFT_SKU_SALE_PACK_UNIT] [SPU] ON ([A].[CODE_SKU] = [SPU].[CODE_SKU])
LEFT JOIN [acsa].[SONDA_PACK_UNIT] [SOPU] ON [SPU].[CODE_PACK_UNIT] = [SOPU].[CODE_PACK_UNIT];
