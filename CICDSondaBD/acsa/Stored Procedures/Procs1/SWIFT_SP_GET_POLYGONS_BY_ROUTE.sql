...
BEGIN
	...
	FROM [$(CICDSondaBD)].[acsa].SWIFT_VIEW_ALL_ROUTE Routes
	INNER JOIN [$(CICDSondaBD)].[acsa].[SWIFT_POLYGON_BY_ROUTE] PbR
		on Routes.ROUTE = PbR.ROUTE
	INNER JOIN [$(CICDSondaBD)].[acsa].[SWIFT_POLYGON] Poly
		on Poly.POLYGON_ID = PbR.POLYGON_ID
  LEFT JOIN [$(CICDSondaBD)].[acsa].[SWIFT_POLYGON] Parent 
