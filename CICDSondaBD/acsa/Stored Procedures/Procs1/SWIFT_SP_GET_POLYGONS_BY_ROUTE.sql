...
BEGIN
	...
	FROM [SWIFT_EXPRESS_R].[acsa].SWIFT_VIEW_ALL_ROUTE Routes
	INNER JOIN [SWIFT_EXPRESS_R].[acsa].[SWIFT_POLYGON_BY_ROUTE] PbR
		on Routes.ROUTE = PbR.ROUTE
	INNER JOIN [SWIFT_EXPRESS_R].[acsa].[SWIFT_POLYGON] Poly
		on Poly.POLYGON_ID = PbR.POLYGON_ID
  LEFT JOIN [SWIFT_EXPRESS_R].[acsa].[SWIFT_POLYGON] Parent 
