...
--  WHERE [RP].[CODE_ROUTE] = @CODE_ROUTE
--  AND [RP].[RELATED_CLIENT_CODE] NOT IN (SELECT
--      [CODE_CUSTOMER]
--    FROM @CUSTOMER)
...
, [CUSTOMER_OWNER])
  ...
  INNER JOIN @CUSTOMER [C]
    ON ([C].[CODE_CUSTOMER] = [PLC].[CODE_CUSTOMER])
  WHERE (@SELLER_OWNER IS NULL
  OR @SELLER_OWNER = '')
  OR [PLC].[OWNER] = @SELLER_OWNER
  ORDER BY [PLC].[CODE_CUSTOMER]

IF (@SELLER_OWNER IS NOT NULL
  OR @SELLER_OWNER != '')
BEGIN
PRINT '--> @SELLER_OWNER: ' + @SELLER_OWNER
--
INSERT INTO @PRICE_LIST_BY_CUSTOMER ([CODE_ROUTE]
, [CODE_PRICE_LIST]
, [CODE_CUSTOMER])
  SELECT
    [CODE_ROUTE]
  ...
  FROM @PRICE_LIST
--
INSERT INTO @PRICE_LIST_BY_SKU_PACK_SCALE ([CODE_ROUTE]
, [CODE_PRICE_LIST]
, [CODE_SKU]
, [CODE_PACK_UNIT]
, [PRIORITY]
...
, [CODE_CUSTOMER])
  SELECT
   ...
   ,[PL].[CODE_CUSTOMER]
  FROM @PRICE_LIST [PL]
  INNER JOIN @CUSTOMER_JUST_ONCE [C]
    ON ([C].[CODE_CUSTOMER] = [PL].[CODE_CUSTOMER])
--
INSERT INTO @PRICE_LIST_BY_SKU_PACK_SCALE ([CODE_ROUTE]
, [CODE_PRICE_LIST]
, [CODE_SKU]
...
--
INSERT INTO @PRICE_LIST_TEMP ([OWNER]
, [CODE_ROUTE]
, [CODE_PRICE_LIST]
, [CODE_CUSTOMER]
, [ORIGINAL_CODE_PRICE_LIST])
...
, [CODE_PRICE_LIST]
, [CODE_CUSTOMER])
  VALUES (@CODE_ROUTE  -- CODE_ROUTE - varchar(50)
...
, [PRICE])
  ...
  INNER JOIN @PRICE_LIST_TEMP [PLT]
    ON (
        [PLT].[OWNER] = [SPS].[OWNER]
  ...
  INNER JOIN [acsa].[SWIFT_VIEW_ALL_SKU] [S]
    ON (
      [S].[CODE_SKU] = [SPS].[CODE_SKU] COLLATE DATABASE_DEFAULT
      )
...
WHERE [ORIGINAL_CODE_PRICE_LIST] = @ORIGINAL_CODE_PRICE_LIST

-- ------------------------------------------------------------------------------------
-- Se obtiene los productos restantes
-- ------------------------------------------------------------------------------------
INSERT INTO @PRICE_LIST_BY_SKU_PACK_SCALE_TEMP ([CODE_ROUTE]
, [CODE_PRICE_LIST]
, [CODE_SKU]
, [CODE_PACK_UNIT]
, [PRIORITY]
...
, [OWNER])
  ...
  LEFT JOIN @PRICE_LIST_BY_SKU_PACK_SCALE [SPST]
    ON (
...
-- ------------------------------------------------------------------------------------
INSERT INTO @SKU_JUST_ONCE ([CODE_SKU]
, [CODE_PACK_UNIT])
  SELECT
    [SPS].[CODE_SKU]
   ,[SPS].[CODE_PACK_UNIT]
  FROM @PRICE_LIST_BY_SKU_PACK_SCALE_TEMP [SPS]
  GROUP BY [SPS].[CODE_SKU]
          ,[SPS].[CODE_PACK_UNIT]
  HAVING COUNT(*) = 1
--
INSERT INTO @PRICE_LIST_BY_SKU_PACK_SCALE ([CODE_ROUTE]
...
, [PRICE])
  ...
  INNER JOIN [acsa].[SWIFT_VIEW_ALL_SKU] [S]
    ON (
        [S].[OWNER] = [SPS].[OWNER] COLLATE DATABASE_DEFAULT
        AND [S].[CODE_SKU] = [SPS].[CODE_SKU] COLLATE DATABASE_DEFAULT
      )
  WHERE [PLT].[CODE_CUSTOMER] = @CODE_CUSTOMER
...
-- ------------------------------------------------------------------------------------
INSERT INTO [acsa].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] ([CODE_ROUTE]
, [CODE_PRICE_LIST]
, [CODE_CUSTOMER])
  SELECT DISTINCT
    [CODE_ROUTE]
...
--
INSERT INTO [acsa].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE] ([CODE_ROUTE]
, [CODE_PRICE_LIST]
, [CODE_SKU]
, [CODE_PACK_UNIT]
, [PRIORITY]
