CREATE PROCEDURE [acsa].[SWIFT_SP_INSERT_INVOICE]
...
DECLARE 
	...
	BEGIN TRY
		...
		--
		SELECT @CODE_CLIENT = H.CODE_CLIENT FROM [acsa].[SWIFT_PICKING_HEADER] H WHERE H.PICKING_HEADER = @PICKING_HEADER
		--
		EXEC [acsa].[SWIFT_SP_GET_NEXT_SEQUENCE] 'INVOICE_DRAFT',@ID OUTPUT
		--
		SET @ID = (@ID*-1)
		...
		INNER JOIN [acsa].[SWIFT_PRICE_LIST_BY_CUSTOMER] C ON (C.CODE_CUSTOMER = @CODE_CLIENT)
		INNER JOIN [acsa].[SWIFT_PRICE_LIST_BY_SKU] S ON (C.CODE_PRICE_LIST = S.CODE_PRICE_LIST AND S.CODE_SKU = D.CODE_SKU)
		WHERE PICKING_HEADER = @PICKING_HEADER
		AND D.SCANNED > 0
		
		SELECT @TOTAL_AMOUNT = SUM(TOTAL_LINE) FROM [acsa].[SONDA_POS_INVOICE_DETAIL] WHERE INVOICE_ID = @ID
		--
