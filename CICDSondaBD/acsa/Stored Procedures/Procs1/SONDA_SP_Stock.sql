...
-- Ejemplo de Ejecucion:
				...
				--
				exec [acsa].[SONDA_SP_Stock] @Warehouse = '30300', @pRESULT = @pRESULT OUTPUT
				--
				SELECT @pRESULT
*/
-- =============================================
...
--
		DELETE FROM
			[acsa].[SONDA_POS_SKUS] -- TABLA REMPLAZADA PARA PRUEBAS, TABLA ORIGINAL SONDA_POS_SKUS
		WHERE
			[ROUTE_ID] = @Warehouse
		OPTION
...
--
		...
		BEGIN
			...
			INSERT	INTO  [acsa].[SONDA_POS_SKUS] --TABLA REMPLAZADA PARA PRUEBAS
					(
						[SKU]
						,[SKU_NAME]
						,[SKU_PRICE]
			...
			--,i.SKU_DESCRIPTION NombreHijo				 		
				...
				,'Manual' AS [SALES_PACK_UNIT]
				,SUM([i].[ON_HAND]) as [INITIAL_QTY]
