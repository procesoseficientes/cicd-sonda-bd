...
-- Ejemplo de Ejecucion:
		EXEC [acsa].[SWIFT_SP_CREATE_RECPTION_HEADER_BY_PICKING]
			@PICKING_HEADER = 1020
		---------------------------------------------------------
...
CREATE PROCEDURE [acsa].[SWIFT_SP_CREATE_RECPTION_HEADER_BY_PICKING]
(
	@PICKING_HEADER INT
)
AS
BEGIN
	...
	-- ------------------------------------------------------------------------------------
	BEGIN TRY
	INSERT INTO [acsa].[SWIFT_RECEPTION_HEADER] (
		TYPE_RECEPTION
		,CODE_PROVIDER
		,CODE_USER
		,DOC_SAP_RECEPTION
	...
	SELECT 'PICKING' AS TYPE_RECEPTION
			...
			,PH.PICKING_HEADER
		FROM [acsa].[SWIFT_PICKING_HEADER] AS PH
		INNER JOIN [acsa].[SWIFT_CLASSIFICATION] AS SC ON(
