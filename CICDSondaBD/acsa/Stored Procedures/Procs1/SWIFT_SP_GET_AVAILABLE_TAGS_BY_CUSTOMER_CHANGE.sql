...
-- EJEMPLO DE EJECUCION: 
		EXEC [acsa].[SWIFT_SP_GET_AVAILABLE_TAGS_BY_CUSTOMER_CHANGE]
		 @CUSTOMER = 3
=========================================================*/
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_AVAILABLE_TAGS_BY_CUSTOMER_CHANGE]
(
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT [T].[TAG_COLOR]
			,[T].[TAG_VALUE_TEXT]
		FROM [acsa].[SWIFT_TAGS] AS T
		WHERE [T].[TAG_COLOR] 
			NOT IN(SELECT DISTINCT [TXC].[TAG_COLOR] 
			FROM [acsa].SWIFT_TAG_X_CUSTOMER_CHANGE AS TXC 
