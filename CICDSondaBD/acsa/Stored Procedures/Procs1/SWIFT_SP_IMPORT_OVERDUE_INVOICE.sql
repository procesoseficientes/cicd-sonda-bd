        ...
        SELECT @QUERY
            = '
				INSERT INTO [#OVERDUE_INVOICE_BY_CUSTOMER]
					...
					FROM (
						SELECT
							INV.DOCUMENT_NO INVOICE_ID
							,INV.ENTRY_NO DOC_ENTRY
							,INV.CUSTOMER_NO CODE_CUSTOMER
							,INV.OrderDate CREATED_DATE
							,INV.DueDate DUE_DATE
							,INV.AMOUNT TOTAL_AMOUNT
							,INV.REMAINING_AMOUNT as PENDING_TO_PAID
						FROM SWIFT_INTERFACES_ONLINE_R.ACSA.ERP_VIEW_INVOICE_PENDING_TO_PAY AS INV
						where GETDATE() > INV.DueDate
						AND INV.AMOUNT > 0
				) AS IV
