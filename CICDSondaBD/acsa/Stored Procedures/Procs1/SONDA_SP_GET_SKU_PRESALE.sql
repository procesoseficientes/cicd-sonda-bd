...
-- Ejemplo de Ejecucion:
        EXEC [acsa].[SONDA_SP_GET_SKU_PRESALE] @WAREHOUSES = '30300', @CODE_ROUTE = 'RP-27'
*/
-- =============================================
...
BEGIN
	...
	INNER JOIN [acsa].[SWIFT_PRICE_LIST_BY_CUSTOMER] [splbc] ON ([srp].[RELATED_CLIENT_CODE] = [splbc].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT);
	--
	...
	SELECT DISTINCT
		...
		,[S].[OWNER_ID]
	FROM [#PRESALE_SKU] [I]
	INNER JOIN [acsa].[SWIFT_PRICE_LIST_BY_SKU] [splbs] ON ([I].[SKU] = [splbs].[CODE_SKU] COLLATE DATABASE_DEFAULT)
